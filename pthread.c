...
void *fibSeq(void *threadid)
{
   ...
   syslog(LOG_NOTICE,"\nFib Seq of thread %d of threadid:%d created at %lf",threadid,syscall(224),readTOD());
   FIB_TEST(seqIterations, Iterations);

   //printf("\nFibonacci(%llu)=%llu\n", seqIterations, fib);
   syslog(LOG_NOTICE,"\nFibSeq %d of thread id:%d stopping at %lf", threadid,syscall(224),readTOD());
...
void *testThread(void *threadid)
{
   double DT=0.0;
   int i;
   syslog(LOG_NOTICE," Test Thread called from main thread of id %d",getpid());
   ...
   syslog(LOG_NOTICE," Parallel Thread %d of test process id :%d created at %lf",i,getpid(),readTOD());
   }
   for(i=0;i<NUM_THREADS;i++)
       {
        pthread_join(threads[i], NULL);
        syslog(LOG_NOTICE,"Joining thread %d at %lf",i,readTOD());
       } 
...
int main (int argc, char *argv[])
{
   ...
   syslog(LOG_NOTICE,"Inside main( ) of id:%d , finished join of test thread",getpid());
